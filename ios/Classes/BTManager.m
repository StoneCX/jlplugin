...
- (void)setupNotifications {
  ...
  // Note: You may need to adjust notification names based on actual SDK
  // implementation
}
...
- (void)connectToDevice:(JL_EntityM *)device {
  ...
  if ([self.delegate
          respondsToSelector:@selector(btManagerConnectionStatusChanged:)]) {
...
- (void)handleDeviceDiscovered:(NSNotification *)notification {
  ...
  if ([self.delegate
          respondsToSelector:@selector(btManagerDidDiscoverDevices:)]) {
    [self.delegate btManagerDidDiscoverDevices:self.mFoundArray];
  }

  // If auto-reconnect is enabled and we have a UUID, check if our device is
  // found
  if (self.autoReconnect && self.mBleUUID &&
      self.connectionStatus == BTConnectionStatusDisconnected) {
    JL_EntityM *device = [self deviceWithUUID:self.mBleUUID];
...
- (void)handleDeviceConnected:(NSNotification *)notification {
  ...
  if ([self.delegate
          respondsToSelector:@selector(btManagerDidConnectDevice:)]) {
...
- (void)handleDeviceDisconnected:(NSNotification *)notification {
  ...
  if ([self.delegate
          respondsToSelector:@selector(btManagerDidDisconnectDevice:)]) {
